...
from tasks.runstraight import task
...
def build_env(enable_randomizer, enable_rendering, version=0, mode='train', control_mode='torque'):
    ...
    sensors = [
        ...
        HistoricSensorWrapper(NormalizeSensorWrapper(
            wrapped_sensor=robot_sensors.IMUSensor(channels=["R", "P", "Y", "dR", "dP", "dY"])), num_history=3),
        HistoricSensorWrapper(NormalizeSensorWrapper(wrapped_sensor=robot_sensors.ToeTouchSensor(laikago.NUM_LEGS)),
                              num_history=3),
        HistoricSensorWrapper(NormalizeSensorWrapper(
    ...
    task = eval('task.RunstraightTaskV{}'.format(version))(mode=mode)
