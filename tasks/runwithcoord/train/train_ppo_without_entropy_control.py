...
if __name__ == "__main__":
    ...
    save_path = os.path.join(TASK_DIR, "saves", "ppo_no_entropy-" + TASK_NAME)
    ...
    if FILE_NAME is not '':
        ...
        if not IF_COORD:
            x = torch.zeros(HID_SIZE, 9).to(torch.device('cuda'))
            pretrain_model['base.0.weight'] = torch.cat((pretrain_model['base.0.weight'], x), 1)
